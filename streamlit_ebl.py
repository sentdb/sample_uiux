...
class findSystem():
    ...
    def nounsVerbs(texts):
        ...
        s = ""
        for i in df_words["nn"].head(10):
            s += str(i)+" + "

        s1 = ""
        for i in df_words1["vb"].head(10):
            s1 += str(i)+" + "

        s2 = ""
        for i in world[0].head(10):
            s2 += str(i)+" + "

        congruence = "[[ " +s+ "]] .X " +" ≡ " + "[[ " +s1+ "]]" + " mod ( " + "[[ " +s2+ "]]"+ " ) "
        print(congruence)

        return world, df_words,df_words1,nn_sumup,vb_sumup,sum_all,s1,s2,equationEstimated,congruence
...
if __name__ == '__main__':
   ...
   df,df1,df2,nn_sumup,vb_sumup,sum_all,s1,s2,equationEstimated,congruence = findSystem.nounsVerbs(texts)

   systemSolution=findSystem.linearCongruence(nn_sumup, vb_sumup, sum_all)

   print ("X: ",systemSolution)


   trends = trend.statcalc(df1,df2)

   print(f"chi2 statistic:     {trends.statistic}")                                             
   print(f"p-value:            {trends.pvalue}")                                                
   print(f"degrees of freedom: {trends.dof}")                                                   
   print("expected frequencies:")                                                               
   print(trends.expected_freq)

   diversity = trend.shannonDiversity(df)
